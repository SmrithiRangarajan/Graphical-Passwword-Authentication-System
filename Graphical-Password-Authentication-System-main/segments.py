...
def start(window):
    ...
    label = Label(segments_frame, text="Please select according to the OTP sent", font=('Calibri', 20))
    ...
    while True:
        ...
        if utils.checkAllClicked(imgClickData):
            ...
            if (sortedClickList[0].id == 1) and (sortedClickList[1].id == 2) and (sortedClickList[2].id == 3) and (
                ...
                utils.create_popup(msg="Successfully Authenticated", font="Gabriola 28 bold")
            else:
                utils.create_popup(msg="authentication denied", font="Gabriola 28 bold")
